...
//   
  const handleOrderButtonClick = async () => {
    if (orderInfo.deliveryAddress.trim().length === 0) {
      ...
      try {
        const res = await orderController.handlerMakeOrder({ ...orderInfo, paymentMethod });
        
        const { status, data } = res;

        if (status&&paymentMethod=="cod") {
          toast.success(
            `Đặt hàng thành công. Nhớ kiểm tra mail nhé`,
            {
              position: "top-right",
              autoClose: 5000,
              closeOnClick: true,
          ...
          toast.error(
            `Có gì trục trặc rồi thử lại sau`,
            {
              position: "top-right",
              autoClose: 5000,
              closeOnClick: true,
    ...
    <div className="has_cart_total_price flex_center">
      ...
      <div className="order_success_payment_method">
        ...
        <div className="order_success_payment_method_item">
          <input onChange={()=>setPaymentMethod("cod")} checked={paymentMethod =="cod" ? "checked":false} type="radio" name="method_payment" className="mg_r_5" />
          <i class="fa-solid fa-file-invoice-dollar mg_r_5 color_primary"></i>
          <label>Thanh toán khi nhận hàng</label>
        </div>
        <div className="order_success_payment_method_item">
          <input onChange={()=>setPaymentMethod("cod1")} checked={paymentMethod =="cod1" ? "checked":false} type="radio" name="method_payment" className="mg_r_5" />
        ...
        <div className="order_success_payment_method_item" style={{marginTop:"1.5rem",}}>
        ...
        <DigitalWalletBox className="digitalWalletBox">
          <div className="digitalWalletBox__paypad">
            <input onChange={()=>setPaymentMethod("paypal")} checked={paymentMethod =="paypal" ? "checked":false} type="radio" name="method_payment" className="mg_r_5" />
            <LazyLoadImage src="https://subiz.com.vn/blog/wp-content/uploads/2014/05/paypal.jpg" />
          </div>
          <div className="digitalWalletBox__vnpay">
            <input onChange={()=>setPaymentMethod("vnpay")} checked={paymentMethod =="vnpay" ? "checked":false} type="radio" name="method_payment" className="mg_r_5" />
      ...
      <div className="has_cart_total_price_text flex">
        <span>Tổng tiền</span>
        <span className="total_price">{toVND(cartInfo.total)}</span>
      </div>
